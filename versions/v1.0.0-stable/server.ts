...
// Hulpfunctie: ElevenLabs Audio Generatie via REST API
...
async function generateFreeSpeechAudio(text: string): Promise<string | null> {
  try {
...
async function generateTTSAudio(text: string, settings: any): Promise<string | null> {
  if (settings.ttsEngine === "elevenlabs") {
    const elAudio = await generateElevenLabsAudio(text, settings);
    if (elAudio) return elAudio;
  }
  return await generateFreeSpeechAudio(text);
}
...
addLog("system", "Hélène Kiosk Server gestart", `Poort ${PORT}`);
...
app.get("/api/health", (req, res) => {
  res.json({ status: "ok", app: "Hélène Kiosk" });
});
...
wss.on("connection", (clientWs) => {
  ...
  async function handleStreamingTurn(audioBase64?: string, customInstruction?: string, modelOverride?: string) {
    try {
      const currentSettings = getSettings();
      const systemInstruction = customInstruction || currentSettings.systemInstruction;
      const activeModel = modelOverride || currentSettings.modelName || "gemini-2.5-flash";
      console.log(`[SERVER] Turn verwerken met Gemini streaming (${activeModel})...`);
      ...
      if (hasValidAudio) {
        ...
        console.log(`[SERVER] Valid WAV audio buffer sent to Gemini: ${wavBase64.length} chars base64`);
        ...
        try {
          ...
          console.warn("[SERVER] Fout bij STT transcriptie:", sttErr);
        }

        const isSpeechFound = userTranscription && userTranscription !== "[Geen verstaanbare spraak]";
        ...
        if (isSpeechFound) {
          ...
          parts.push({
            text: `${systemInstruction}\n\nDe gebruiker heeft ingesproken: "${userTranscription}". Geef direct een inhoudelijk, enthousiast, vriendelijk antwoord in het Nederlands op wat de gebruiker vraagt (maximaal 2 zinnen).`,
          });
        } else {
          console.log("[SERVER] Geen verstaanbare spraak gedetecteerd in audio.");
          addLog("user", "⚠️ Gebruiker: [Geen verstaanbare spraak of stilte opgevangen]");
          parts.push({
            inlineData: {
              mimeType: "audio/wav",
              data: wavBase64,
          ...
          parts.push({
            text: `${systemInstruction}\n\nLuister naar de ingesproken audio in de WAV bijlage. Geef een kort, vriendelijk antwoord in het Nederlands (maximaal 2 zinnen).`,
          });
        }
      } else {
        ...
        parts.push({
          text: `${systemInstruction}\n\nIemand heeft de knop ingedrukt om met je te praten. Geef een hele korte, enthousiaste begroeting in het Nederlands en vraag waarmee je ze kunt helpen op het scoutingkamp.`,
        });
      }
  ...
  clientWs.on("message", async (rawMessage) => {
    try {
      ...
      if (data.type === "start_session") {
        ...
        addLog("system", "TV Kiosk WebSocket sessie gestart");
        if (session) {
...
async function startServer() {
  ...
  server.listen(PORT, "0.0.0.0", () => {
    console.log(`[SERVER] Hélène Kiosk server actief op http://0.0.0.0:${PORT}`);
  });
}
